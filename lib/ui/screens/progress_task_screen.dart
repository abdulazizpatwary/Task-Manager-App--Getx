...
class _ProgressTaskScreenState extends State<ProgressTaskScreen> {
  TaskController progressTaskController = Get.find<TaskController>();
  @override
  void initState() {
  ...
  Future<void>taskListByStatus()async{
    bool isSuccess =await progressTaskController.getTaskListByStatus(status: TaskStatus.progressTask);
    if(isSuccess){
      snackBarMessage(context, message: 'Succeess');
    }else{
      snackBarMessage(context, message: progressTaskController.errorMsg!);
    }
  }
